...
function popupBox(treeNode) {
    ...
    var options = {};
    if (_SIGN == "add") {
        options = {area: ['555px', '555px'], id: 'box'};
        title = "新增" + (treeNode.level + 1) + "级分类";
    } else if (_SIGN == "edit") {
        options = {area: ['555px', '297px'], id: 'box'};
...
function splitFormatSecondary() {
    var flag = splitFormat($("#categoryClass").val().trim());
    if (typeof flag == "string") {
        if (flag.search("-1") != -1) {
            var num = flag.substring(flag.indexOf(',') + 1, flag.length);
            $.layer.alert("第" + num + "行分类名称不可超出20个字！");
            return false;
        }
    }
